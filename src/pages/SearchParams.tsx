...
const SearchParams = () => {
  ...
  const times = ["6:30 - 7 PM", "7:00 - 7:30 PM", "7:30 - 8 PM", "8:30 - 9 PM","9:30 - 10 PM"];
    ...
    <>
      <div className="mx-4 md:mx-8 relative">
        {isMenuOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
            <div
              ...
              style={{
                boxShadow:
                  "0px 175px 70px rgba(0, 0, 0, 0.02), 0px 98px 59px rgba(0, 0, 0, 0.07), 0px 44px 44px rgba(0, 0, 0, 0.12), 0px 11px 24px rgba(0, 0, 0, 0.14)",
              }}
            >
              {/* Close Button */}
              <button
              ...
              >
                <div className="relative w-5 h-5">
                  <img
                    src={close}
                    alt="close"
                    className="absolute w-5 h-5 text-[#5BB834] transition-transform duration-300"
                  />
                </div>
              </button>

              <div className="flex flex-col justify-center items-center gap-6 w-[90%] max-w-[430px] h-[496px] mt-12 mx-auto my-10 py-5 overflow-y-auto">
        ...
        {isDeliveryModalOpen && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black bg-opacity-30 ">
            <div
              ...
              style={{
                boxShadow:
                  "0px 83.646px 33.1271px rgba(0, 51, 13, 0.01), 0px 47.2062px 28.1581px rgba(0, 51, 13, 0.05), 0px 20.7045px 20.7045px rgba(0, 51, 13, 0.09), 0px 4.96907px 11.5945px rgba(0, 51, 13, 0.1)",
              }}
            >
              <button
              ...
              <div className="p-6">
                ...
                <div className="flex justify-between mb-8">
                  {dates.map((item) => (
                    ...
                    >
                      <div className={`py-2 px-6 rounded-md `}>
                        <span
                ...
                <div className="mb-8 max-h-36 overflow-y-auto s pr-2">
                  {times.map((time) => (
                    <div
                      key={time}
                      className={`flex justify-center items-center py-3 px-8 rounded-md cursor-pointer ${
                        selectedTime === time ? " text-[#5BB834]" : "opacity-40"
                      }`}
                      onClick={() => setSelectedTime(time)}
                    >
                      <span
                        className={`text-lg ${
                          selectedTime === time ? "font-medium" : "font-normal"
                        }`}
                      >
                        {time}
                      </span>
        ...
        <div className="mb-4 flex justify-between items-center p-4">
          ...
          />
          <button
            className="bg-[#5BB834] flex w-full sm:w-[600px] font-bold cursor-pointer rounded-3xl py-3 px-4 text-white transition-colors justify-between items-center mr-[1rem]"
        ...
        <div className="flex justify-between mt-10 sm:mt-14">
          ...
          <button
            onClick={toggleMenu}
            className={`rounded-lg py-2 px-1 border-green-600 border-2 transition-colors ${
              isMenuOpen
                ? "bg-[#5BB834] text-white"
                : "bg-[#5BB8342E] hover:bg-[#5BB8343A]"
            }`}
          >
            Menu
          </button>
